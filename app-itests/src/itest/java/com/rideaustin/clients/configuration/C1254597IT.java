...
/**
 * @see <a href="http://testrail.devfactory.com/index.php?/cases/view/C1254597">C1254597</a>
 */
...
@WebAppConfiguration
public class C1254597IT extends AbstractTransactionalJUnit4SpringContextTests {
  ...
  private String itemKey = C1254597IT.class.getSimpleName();
  ...
  public void before() throws Exception {
    ...
    driverAction.goOffline(driver.getEmail());
  ...
  @Test
  @TestCases("C1254597")
  public void update_existing_driver_configuration_item_when_driver_is_OFFLINE() throws Exception {
    ...
    configurationItemsAdminAction.update(administrator.getEmail(), itemId, "{\"configurationValue\" : {\"some\" : \"new_json\"}}")
      .andDo(print())
      .andExpect(status().isOk());

    mvcResult = configurationItemsDriverAction.getEvents(driver)
      .andDo(print())
      .andExpect(status().isOk())
      .andExpect(request().asyncStarted())
      .andReturn();
    ...
    configurationItemsDriverAction.getAsyncDispatch(mvcResult)
      ...
      .andExpect(status().isOk())
      .andExpect(content().contentType("application/json;charset=UTF-8"))
      .andExpect(jsonPath("$[0].eventType").value(equalTo(EventType.CONFIG_CHANGED.name())))
      .andExpect(jsonPath("$[0].parameters").value(notNullValue()))
    ;
  }
