...
import static com.rideaustin.model.user.RiderCard.FAILED_CHARGE_THRESHOLD;
...
public class RA13663ThresholdExceededIT extends AbstractNonTxTests<RA13663Setup> {
  ...
  public void test() throws Exception {
    ...
    rider.getPrimaryCard().setFailedChargeAttempts(FAILED_CHARGE_THRESHOLD - 1);
    rideDslRepository.saveAny(rider.getPrimaryCard());
    ...
    assertEquals(FAILED_CHARGE_THRESHOLD, cards.get(0).getFailedChargeAttempts());
    assertNotNull(cards.get(0).getLastFailureDate());

    Ride one = rideDslRepository.findOne(ride);
    assertEquals(PaymentStatus.BLOCKED, one.getPaymentStatus());
  }
}
