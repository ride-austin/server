...
public class C1266291IT extends AbstractCarLookupTest {
  ...
  public void testSearchRegular() throws Exception {
    ...
    for (int i = 0; i < drivers.size(); i++) {
      driverAction.locationUpdate(drivers.get(i), center.lat + shift * i, center.lng + shift * i, new String[]{TestUtils.REGULAR});
    }

    List<CompactActiveDriverDto> foundDrivers = riderAction.searchDrivers(rider.getEmail(), center, TestUtils.REGULAR, null);
  ...
  public void testSearchWomenOnly() throws Exception {
    ...
    for (int i = 0; i < drivers.size(); i++) {
      if (i < 10) {
        driverAction.locationUpdate(drivers.get(i), center.lat + shift * i, center.lng + shift * i, new String[]{TestUtils.REGULAR}, new String[]{"WOMEN_ONLY"});
      } else {
        driverAction.locationUpdate(drivers.get(i), center.lat + shift * i, center.lng + shift * i, new String[]{TestUtils.REGULAR});
      }
    }

    List<CompactActiveDriverDto> foundDrivers = riderAction.searchDrivers(rider.getEmail(), center, TestUtils.REGULAR, "WOMEN_ONLY");
    assertEquals(10, foundDrivers.size());
    assertEquals(0L, (long) foundDrivers.get(0).getDrivingTimeToRider());
